...
class DQNAgent():
    ...
    def conv_game_to_state(self, game):
        ...
        state.extend([conv_cards_to_list(temp.copy(), game.deck.discard_pile[-(i+1)]) for i in range(self.discard_memory)])
    ...
    def decide_action(self, nn_input, game):
        ...
        act_values = self.target_NN.predict(nn_input)
    ...
    def copy_train_learn(self):
        target_NN.set_weights(learning_NN.get_weights()) 
    
    def train(self, num_learning_iter=100, batch_update=1000, sample_size=10000):
        ...
        for learn_iter in range(num_learning_iter):
            for game in range(batch_update):
                ...
                while game.current_player != agent_idx:
                    ...
                    other_action = self.other_player(game)
